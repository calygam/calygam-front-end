    ...
    return (
        <button type='button' className='flex justify-center outline-none font-poppins cursor-pointer items-center  w-full group   rounded-l-xl relative   lg:p-4 py-4 px-6    ' onClick={()=> setSelectedButton(identifier)}>
            ...
            <div className='absolute w-full    group h-full flex justify-end'>
                ...
                <div className={`  delay-0 duration-[750ms] w-0 ease-in-out  group-hover:rounded-0 group-hover:w-full   group-hover:h-full bg-white/15 ${identifier === selectedButton? "w-full bg-white/35 ":"w-0 bg-transparent"}`}></div>
            </div>
}
